...
char DESEnc::PC1_Table[PC_1_SIZE] = {
						56,48,40,32,24,16,8,
						0,57,49,41,33,25,17,
						9,1,58,50,42,34,26,
						18,10,2,59,51,43,35,
...
char DESEnc::Move_Table[SUBKEY_NUM] = {23,10,2,5,9,2,3,2,3,2,5,7,2,9,2,7};
 
char DESEnc::PC2_Table[PC_2_SIZE] = {
...
char DESEnc::IP_Table[BIT_STR_SIZE] = {
...
char DESEnc::Expand_Table[EXPAND_SIZE] = { 
						31, 0, 1, 2, 3, 4,
						3,  4, 5, 6, 7, 8,
						7,  8, 9,10,11,12,
						11,12,13,14,15,16,
...
char DESEnc::Permute_Table[BIT_STR_SIZE / 2] = {
						15, 6,19,20,28,11,27,16,
						0,14,22,25, 4,17,30, 9,
						1, 7,23,13,31,26, 2, 8,
						18,12,29, 5,21,10, 3,24};
 
char DESEnc::IP_1_Table[BIT_STR_SIZE] = {
...
char DESEnc::SBox_Table[KEY_SZIE][4][16] = {
						//S1
...
std::string DESEnc::Encrypt(const std::string& plain, const std::string& key)
{
	std::string result;
...
std::string DESEnc::Decrypt(const std::string& cipher, const std::string& key)
{
	std::string result;
...
bool DESEnc::EncryptBlock(std::string& block, char subKey[SUBKEY_NUM][SUBKEY_LENGHT])
{
...
bool DESEnc::DecryptBlock(std::string& block, char subKey[SUBKEY_NUM][SUBKEY_LENGHT])
{
	if(block.size() != KEY_SZIE)
		return false;
...
bool DESEnc::CreateSubKey(const std::string& key, char subKey[SUBKEY_NUM][SUBKEY_LENGHT])
{
...
bool DESEnc::PC1_Transform(const std::string& bitStr, std::string& PC1BitStr)
{
...
bool DESEnc::PC2_Transform(const std::string& PC1BitStr, char subKey[SUBKEY_LENGHT])
{
...
bool DESEnc::IP_Transform(std::string& bitStr)
{
...
bool DESEnc::Expand_Transform(const std::string& halfBitStr, std::string& eBitStr)
{
...
bool DESEnc::SBox_Transform(const std::string& eBitStr, std::string& halfBitStr)
{
...
bool DESEnc::Permute_Transform(std::string& halfBitStr)
{
...
bool DESEnc::IP_1_Transform(std::string& bitStr)
{
	if(bitStr.size() != BIT_STR_SIZE)
		return false;
...
bool DESEnc::Char8ToBit64(const std::string& str, std::string& bitStr)
{
...
bool DESEnc::Bit64ToChar8(const std::string& bitStr, std::string& str)
{
...
bool DESEnc::LeftCycle(std::string& str, size_t beginSection, size_t endSection, size_t step)
{
...
bool DESEnc::XOR(std::string& strFirst, std::string& strSecond, size_t num)
{
